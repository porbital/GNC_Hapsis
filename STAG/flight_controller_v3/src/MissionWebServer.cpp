...
const char* ap_ssid = "Hapsis";
const char* ap_password = "Polaris123";

WebServer server(80);

// Variables to store the latest data for the web dashboard
static float web_altitude = 0.0;
static float web_vertical_vel = 0.0;
...
<body>
  <div class="container">
    ...
    <div id="altitude" class="telemetry">Altitude: -- m</div>
    <div id="vvel" class="telemetry">Vertical Vel: -- m/s</div>
    <br>
    ...
    function updateStatus() {
      fetch('/status')
        ...
        .then(data => {
          ...
          document.getElementById('altitude').innerText = 'Altitude: ' + parseFloat(data.alt).toFixed(2) + ' m';
          document.getElementById('vvel').innerText = 'Vertical Vel: ' + parseFloat(data.vvel).toFixed(2) + ' m/s';
          
          // Only enable LAUNCH when in ARMED state
          document.getElementById('btnLaunch').disabled = (data.state !== 'ARMED');
          // Disable ARM button if already armed or beyond
          const preArm = (data.state === 'GROUND');
...
void handleStatus() {
  // Format data as a simple JSON string including altitude and vertical velocity
  String json = "{\"state\":\"" + String(stateToString(currentState)) + "\",\"alt\":\"" + String(web_altitude, 2) + "\",\"vvel\":\"" + String(web_vertical_vel, 2) + "\"}";
