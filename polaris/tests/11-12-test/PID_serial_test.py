...
# 180 -> South
# 270 -> West
setpoint = 0  # (Degrees) Point north - FIXED, NO CYCLING

# Define heading sequence and timing - DISABLED
# headings = [90, 180, 270]
# heading_index = 0
...
try:
    ...
    blink_count = 0  # Track number of blinks
    max_blinks = 5  # Only blink 5 times
    ...
    # Cycle headings function - DISABLED
    # def cycle_headings():
    ...
    # Send CSV header over serial
    send_data("Time,Angular Position,R Solenoid,L Solenoid,Angular Vel X,Angular Vel Y,Angular Vel Z,PID Error,P,D")
    print("GNC System Started - Sending data via serial")
    ...
    while True:
        ...
        try:
            # Heading cycling disabled - maintaining setpoint at 0 degrees
            # cycle_headings()
            ...
            if abs(output) > threshold:
                if output > 0:  # Fire right thruster
                    ...
                    if not positive_thruster_on:
                        ...
                        red_led.value = False
                        yellow_led.value = False
            # Blink LED (only first 5 times)
            if blink_count < max_blinks and current_time - last_toggle_time >= blink_interval:
                led_state = not led_state  # Toggle LED
                green_led.value = led_state
                last_toggle_time = current_time  # update toggle
                if led_state:  # Count only when turning on
                    blink_count += 1

            # Send Data via Serial (replaces SD card logging)
            data_line = f"{delta_time},{current_heading},{int(negative_thruster_on)},{int(positive_thruster_on)},{gyro_x},{gyro_y},{gyro_z},{output},{error},{derivative}"
            send_data(data_line)
